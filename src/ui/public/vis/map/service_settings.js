      ...
      _invalidateSettings() {
        ...
        this._loadCatalogue = _.once(async () => {
          ...
          try {
            ...
            var r = await this._getManifest(mapConfig.manifestServiceUrl, this._queryParams);
            
            //if(manifestUrl = "https://tiles.maps.elastic.co/v2/manifest"){
			
			//"https://tiles.maps.elastic.co/v2/default/{z}/{x}/{y}.png?elastic_tile_service_tos=agree&my_app_name=kibana"
			/*
			r.data.services[0].maxzoom = 18;
			r.data.services[0].url = "http://tile.openstreetmap.org/{z}/{x}/{y}.png";
			console.log(mapConfig.manifestServiceUrl);
            console.log(r);
            */
            const response = r;
            return response.data;
