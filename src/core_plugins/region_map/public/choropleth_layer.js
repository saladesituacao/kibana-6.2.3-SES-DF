...
//editado por edmar moretti
  constructor(geojsonUrl, attribution, format, showAllShapes, meta, numeroDeClasses) {
    super();
    this._metrics = null;
    this._joinField = null;
    this._numeroDeClasses = numeroDeClasses;
  ...
  _setStyle() {
    ...
    if (this._metrics && this._metrics.length > 0) {
      ...
      this._legendColors = getLegendColors(this._colorRamp,this._numeroDeClasses);
...
//editado por edmar moretti
  setNumeroDeClasses(n) {
    if (this._numeroDeClasses === n) {
      return;
    }
    this._numeroDeClasses = n;
    this._setStyle();
  }
...
function getLegendColors(colorRamp,numeroDeClasses) {
  if(numeroDeClasses == undefined){
    numeroDeClasses = 5;
  }
  ...
  */
  const l = colorRamp.length;
  colors[0] = getColor(colorRamp, 0);
  for(var i = 1; i < (numeroDeClasses - 1); i++){
    colors[i] = getColor(colorRamp, Math.floor(l * i / numeroDeClasses));
  }
  colors[numeroDeClasses - 1] = getColor(colorRamp, l - 1);
  return colors;
}
