    ...
    if(resp){
      ...
      if($scope.vis.aggs.bySchemaName['x-axis']){
          ...
          if($scope.vis.aggs.bySchemaName['x-axis'][0].type.name != "count"){
            //console.log($scope.vis.aggs.bySchemaName['x-axis']);
            //editado por edmar moretti
            //var metricsAgg_xAxis_name = $scope.vis.aggs.bySchemaName['x-axis'][0].params.field.displayName;
            var metricsAgg_xAxis_name = $scope.vis.aggs.bySchemaName['x-axis'][0].params.customLabel;
          }else{
      ...
      if($scope.vis.aggs.bySchemaName['y-axis']){
          ...
          if($scope.vis.aggs.bySchemaName['y-axis'][0].type.name != "count"){
            ...
            var metricsAgg_yAxis_name = $scope.vis.aggs.bySchemaName['y-axis'][0].params.customLabel;
          }else{
      ...
      var dataParsed = resp.tables[0].rows.map(function(bucket) {
        ...
        //console.log(bucket)
        //console.log(bucket[id_firstfield])
        return {
          mode: 'markers',
      ...
      Plotly.newPlot('dotplot-graph', data, layout, {modeBarButtonsToRemove: ['toImage','sendDataToCloud','autoScale2d','resetScale2d','hoverClosestCartesian','hoverCompareCartesian','toggleSpikelines'], showLink: false, displayModeBar: true, displaylogo: false})
    }
